...
export async function processTextCapture(userId: string, data: TextCaptureData) {
...
export async function processVoiceCapture(userId: string, data: VoiceCaptureData) {
...
export async function processImageCapture(userId: string, data: ImageCaptureData) {
...
export async function processQueuedCaptures(userId: string, captures: QueuedCapture[]) {
  const results = [];
  ...
  for (const capture of captures) {
    try {
      ...
      switch (capture.type) {
        case 'text':
          result = await processTextCapture(userId, capture.data);
          break;
        case 'voice':
          result = await processVoiceCapture(userId, capture.data);
          break;
        case 'image':
          result = await processImageCapture(userId, capture.data);
          break;
        default:
